...
print_help() {
    ...
    echo "-N: Teleop Robot"
    echo "-W: MAINTENANCE TELEOP MODE"
...
case "$1" in
    "") 
        ...
        exit 1
        ;;
    -s|-i|-x|-u|-v|-o|-h|-p|-X|-L|-M|-T|-d|-c|-m|-B|-E|-A|-G|-O|-D|-N|-W|-c21|-c22)
        setTarget="$1"
        ;;
    *)
...
# -d debug_detail
   ...
   # -W MAINTENANCE TELEOP MODE
   elif [[ $setTarget = "-W" ]]; then
    robotNumber=$2
    robotIP=${robotIP}   # already calculated earlier in script
    echo "=== Maintenance Mode | Robot ID: $robotNumber | IP: $robotIP ==="
    echo "⚡ Starting MAINTENANCE teleop."
    echo "⚠ Robot will NOT move until you press keys."
    echo "⚠ Enter sudo password manually when prompted."
    # SSH → Docker → ROS maintenance teleop
    ssh -t -o StrictHostKeyChecking=no zippy@${robotIP} \
    "sudo docker exec -it zippy bash -c '
        source /home/zippy/zippy_ws/install/setup.bash &&
        echo \"Maintenance Teleop Started\" &&
        rosrun maintenance_teleop maintenance_teleop_key
    '"
    echo "⚡ Maintenance teleop session ended."
