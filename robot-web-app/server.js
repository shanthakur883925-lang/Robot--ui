...
io.on('connection', (socket) => {
    ...
    socket.on('run-command', (data) => {
        ...
        // Telemetry Flags (High-rate topics)
        const TELEMETRY_FLAGS = ['-O', '-d', '-A', '-m', '-E', '-G', '-c', '-B'];
        ...
        child.stdout.on('data', (chunk) => {
            ...
            if (isTelemetry) {
                ...
                if (messages.length > 1) {
                    ...
                    // FIELD FILTERING: For raw_odom (-O), keep only essentials
                    if (flag === '-O' && candidate.includes('pose:')) {
    ...
    // ==========================================
    // MAINTENANCE TERMINAL (Real PTY for lifter control)
    // ==========================================
    let maintenancePty = null;

    socket.on('start-maintenance', (data) => {
        const { robotNumber, robotIP, robotPassword } = data;

        if (maintenancePty) {
            socket.emit('maintenance-output', '\r\n⚠ Maintenance session already running. Stop it first.\r\n');
            return;
        }

        console.log(`Starting maintenance for robot ${robotNumber} at ${robotIP}`);
        socket.emit('maintenance-output', `\r\n🔧 Starting MAINTENANCE session for Robot ${robotNumber}...\r\n`);
        socket.emit('maintenance-output', `📍 Lifter Control: W=UP, X=DOWN\r\n`);
        socket.emit('maintenance-output', `─────────────────────────────────────────\r\n`);

        // The command to run maintenance teleop
        const sshCommand = `sshpass -p ${robotPassword} ssh -t -o StrictHostKeyChecking=no zippy@${robotIP} "sudo docker exec -it zippy bash -c 'source /home/zippy/zippy_ws/install/setup.bash && rosrun maintenance_teleop maintenance_teleop_key'"`;

        // Create PTY
        maintenancePty = pty.spawn('bash', ['-c', sshCommand], {
            name: 'xterm-color',
            cols: 80,
            rows: 24,
            cwd: process.cwd(),
        ...
        maintenancePty.onData((data) => {
            socket.emit('maintenance-output', data);
        });

        maintenancePty.onExit(({ exitCode }) => {
            socket.emit('maintenance-output', `\r\n─────────────────────────────────────────\r\n`);
            socket.emit('maintenance-output', `⚡ Maintenance session ended (exit code: ${exitCode})\r\n`);
            socket.emit('maintenance-stopped');
            maintenancePty = null;
        });
    });

    socket.on('maintenance-input', (data) => {
        if (maintenancePty) {
            maintenancePty.write(data);
        }
    });

    socket.on('stop-maintenance', () => {
        if (maintenancePty) {
            socket.emit('maintenance-output', '\r\n⏹ Stopping maintenance & exiting robot...\r\n');
            // Send Ctrl+C to properly exit the ROS node
            maintenancePty.write('\x03');  // Ctrl+C
            // Then send exit to close the shell
            setTimeout(() => {
                if (maintenancePty) {
                    maintenancePty.write('exit\n');
                }
            }, 300);
            // Wait a moment then kill the PTY
            setTimeout(() => {
                if (maintenancePty) {
                    maintenancePty.kill('SIGINT');
                    maintenancePty = null;
                }
            }, 800);
        }
    ...
    socket.on('resize-maintenance', (data) => {
        if (maintenancePty && data.cols && data.rows) {
            maintenancePty.resize(data.cols, data.rows);
        }
    });
    ...
    socket.on('disconnect', () => {
        ...
        // Clean up maintenance PTY on disconnect
        if (maintenancePty) {
            maintenancePty.kill();
            maintenancePty = null;
        }
