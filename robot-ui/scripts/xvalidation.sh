...
case "$1" in
    "") 
        ...
        exit 1
        ;;
    -s|-i|-x|-u|-v|-o|-h|-p|-X|-L|-M|-T|-d|-c|-m|-B|-E|-A|-G|-O|-D|-Z)
        setTarget="$1"
        ;;
    *)
...
# -T Teleop Robot
    ...
    commandA="source /home/zippy/zippy_ws/install/setup.bash"
    commandB="echo ''"
    commandC="echo STARTING TELEOP MODE"
    commandD="rosrun turtlebot3_teleop turtlebot3_teleop_key"
    
    dockerCommand="sudo -S docker exec -it zippy bash -c '${commandA} && ${commandB} && ${commandC} && ${commandD}'"
    
    sshpass -p ${robotpassword} ssh -tt -o StrictHostKeyChecking=no zippy@${robotIP} "echo ${robotpassword} | ${dockerCommand}"
